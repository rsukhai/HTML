        ...
        Array(halfCount)
          ...
          .map((_el, index) => {
            ...
            return (<div
              ...
              style={{ 'background-color': allDatesStr.includes(showYear + 1 + "") ? 'green' : 'rgba(0, 0, 0, 0)' }}
            >{showYear + 1}</div>);
          })
      }
      <div className='year content-item' onClick={() => click(year)} >{year}</div>
      {
        Array(halfCount)
          ...
          .map((_el, index) => {
            ...
            const allDatesStr = datesWithEvents.map(date => date.year)
            return (
              <div style={{ 'backcolor': allDatesStr.includes(showYear + 1 + "") ? 'green' : 'white' }} className='year content-item' onClick={() => click(showYear)} >
                {showYear}
              </div>);
          })
      }
